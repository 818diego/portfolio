...
export const Hero: React.FC = () => {
  ...
  useGsapScope(() => {
    ...
    const indicator = sectionRef.current?.querySelector(
  ...
  return (
    ...
    >
      <div className="container mx-auto px-6 py-24 text-center relative">
        <div
          data-hero
          className="inline-flex items-center gap-2 bg-green-900/30 px-4 py-2 rounded-full mb-6"
        >
          <BsTerminalFill className="w-4 h-4 text-green-400 shrink-0" />
          <span className="text-green-400 font-medium">
            {t("personal.available")}
          </span>
          <span className="text-green-400/40" aria-hidden="true">
            ·
          </span>
          <a
            href="#contact"
            className="text-green-300 font-semibold hover:text-green-200 hover:underline underline-offset-2 cursor-pointer"
          >
            {t("hero.contactMe")} →
          </a>
        </div>
      ...
      <div className="absolute bottom-16 left-1/2 transform -translate-x-1/2 z-20">
        <div
          data-scroll-indicator
          className="flex flex-col items-center text-green-400 p-4"
        >
