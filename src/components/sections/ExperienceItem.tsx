    ...
    return (
        ...
        >
            ...
            <div className={`w-full md:w-1/2 pl-10 md:pl-0 ${isEven ? 'md:pr-12' : 'md:pl-12'}`}>
                <div 
                    className="group bg-white dark:bg-zinc-800 rounded-xl p-5 shadow-sm dark:shadow-zinc-900 border border-gray-100 dark:border-zinc-700/50 transition-all duration-300"
                >
                    <div className="flex flex-col mb-3">
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                            ...
                            <div className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-green-500/10 dark:bg-zinc-700/50 border border-green-500/30 dark:border-zinc-600/30 transition-all duration-300">
                                <Briefcase className="w-3 h-3 text-green-500 dark:text-gray-400" />
                                <span className="font-bold text-[10px] uppercase tracking-wider text-gray-700 dark:text-white">{exp.company}</span>
                            </div>
                        </div>
                        <h3 className="text-xl font-black text-green-600 dark:text-green-400 transition-colors">
                            {exp.title}
                        </h3>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 overflow-hidden transition-all duration-700 ease-in-out opacity-100">
                        {exp.achievements.map((achievement: any, i: number) => (
                            <div
                                key={i}
                                className="flex items-center px-3 py-2 rounded-lg bg-green-500/20 dark:bg-green-400/5 text-green-800 dark:text-green-300 border border-green-500/30 dark:border-green-400/20 transition-all duration-300"
                            >
