...
export const ContactInfo: React.FC = () => {
  ...
  useGsapScope(() => {
    ...
    if (prefersReducedMotion()) {
      ...
      staggerReveal(left as unknown as gsap.DOMTarget, {
        x: -20,
        stagger: 0.08,
        duration: 0.6,
      });
      staggerReveal(right as unknown as gsap.DOMTarget, {
        x: 20,
        stagger: 0.08,
        duration: 0.6,
        delay: 0.1,
  ...
  return (
    <div
      ref={rootRef}
      className="max-w-full mx-auto bg-zinc-800 rounded-xl shadow-sm border border-zinc-700/50 overflow-hidden flex flex-col md:flex-row"
    >
      <div
        data-ci-left
        className="flex flex-col justify-center p-8 w-full md:w-1/2 bg-zinc-900/20"
      >
        ...
        <div className="mt-8 space-y-4">
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <span className="text-[12px] font-bold text-blue-400/70 uppercase tracking-widest">
          ...
          <div
            data-ci-left
            className="flex items-center justify-between p-4 rounded-xl bg-orange-500/5 border border-orange-500/10"
          >
            <div className="flex items-center gap-4">
              ...
              <span className="text-[12px] font-bold text-orange-400/70 uppercase tracking-widest">
                {t("contact.spokenLanguages")}
              </span>
            </div>
            <span className="text-sm text-zinc-300 font-bold">
              {t("contact.spokenLanguagesValue")}
            </span>
          </div>
          <div
            data-ci-left
            className="flex items-center justify-between p-4 rounded-xl bg-zinc-500/5 border border-zinc-500/10"
          >
            <div className="flex items-center gap-4">
              ...
              <span className="text-[12px] font-bold text-zinc-400/70 uppercase tracking-widest">
                {t("contact.timezone")}
              </span>
            </div>
      ...
      <div data-ci-right className="p-8 w-full md:w-1/2 relative">
        ...
        >
          <div data-ci-right>
            <label
              className="block text-[13px] font-bold text-green-400 uppercase tracking-wider mb-2"
              htmlFor="name"
            >
              {t("contact.name")}
            </label>
          ...
          <button
            ...
            disabled={isSubmitting}
            className="w-full py-2.5 px-6 bg-green-500/10 hover:bg-green-500/20 text-green-400 font-bold rounded-lg border border-green-500/20 hover:border-green-500/40 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed group"
          >
            {isSubmitting ? (
